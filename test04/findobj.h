...
void GetGrayImageCamFront(C24BitMap&CPic,C256BitMap &GPic)
{
	int i, j,t;
	if(GPic.Width!= CPic.Width)
		GPic.FormatF(CPic.Width, CPic.Height);
	...
	for (i = 0; i< CPic.Width; i++)
	 for (j = 0; j < CPic.Height; j++)
	 {
        ...
        if( ( fabs(h - 30) < 15 ) &&  (( s * 255.0) > 70 && ( s * 255.0) <150.0 ) && ( v > 80.0) )
		{
			val = 0;
			for(t=-10;t<10;t++)
			{*get_pix_color(GPic, i+t, j) =0;}
...
void (*pGray)(C24BitMap&CPic,C256BitMap &GPic) = NULL;


//
//RegionVec[0].left,RegionVec[0].top,RegionVec[0].right,RegionVec[0].bottom
    ...
    int & Boxleft,int & Boxtop,int & Boxright,int & Boxbottom)
{
	...
	//CPic.Rectangle(RegionVec[0].left,RegionVec[0].top,RegionVec[0].right,RegionVec[0].bottom);
	  Boxleft   = RegionVec[0].left;
	  Boxtop    = RegionVec[0].top;
	  Boxright  = RegionVec[0].right;
	  Boxbottom = RegionVec[0].bottom;
