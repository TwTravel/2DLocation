...
int MergeTxtStrNUM(C24BitMap&Pic, int x,int y,int size, int number,int R,int G,int B) 
{
	char Txt[20];
	sprintf(Txt,"%02i",number);
	int Length =strlen(Txt);
	int i;
	int xx =x;
	for (i=0;i<Length;i++)
...
int MergeTxtStrNUM(C24BitMap&Pic, int x,int y,int size, double number1,double number2,int R,int G,int B) 
{
	char Txt[50];
	sprintf(Txt,"%.1lf,%.1lf", number1, number2);
	int Length =strlen(Txt);
	int i;
	int xx =x;
	for (i=0;i<Length;i++)
